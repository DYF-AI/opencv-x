...
int main()
{
	...
	cv::imwrite("../outputs/histograms_groupBW.jpg", image);
	...
	cv::imwrite("../outputs/histograms_Histogram.jpg", h.getHistogramImage(image));
	...
	cv::imwrite("../outputs/histograms_threshold.jpg", hi);
	...
	cv::imwrite("../outputs/histograms_Binary.jpg", thresholded);

	thresholded = 255 - thresholded;
	cv::imwrite("../outputs/histograms_Binary_2.jpg", thresholded);
	...
	cv::imwrite("../outputs/histograms_Equalized.jpg",eq);

	// Show the new histogram
	// cv::namedWindow("Equalized H");
	// cv::imshow("Equalized H",h.getHistogramImage(eq));
	cv::imwrite("../outputs/histograms_Equalized_H.jpg", h.getHistogramImage(eq));
	...
	cv::imwrite("../outputs/histograms_Stretched.jpg", str);
	

	// Show the new histogram
	// cv::namedWindow("Stretched H");
	// cv::imshow("Stretched H",h.getHistogramImage(str));
	cv::imwrite("../outputs/histograms_Stretched_H.jpg", h.getHistogramImage(str));
	...
	cv::imwrite("../outputs/histograms_Negative.jpg", h.applyLookUp(image,lut));
